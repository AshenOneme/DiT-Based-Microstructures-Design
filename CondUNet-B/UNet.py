...
# class UNet_AttentionBlock(nn.Module):
#     def __init__(self, n_channels: int, n_heads: int, n_dims: int = None, n_groups: int = 32, d_time: int = 128, d_cond: int = 128):
#         super().__init__()
#         if n_dims is None:
#             n_dims = n_channels
#         self.gn = nn.GroupNorm(n_groups, n_channels)
#         self.proj = nn.Linear(n_channels, n_dims * 3)  # For Q, K, V
#         self.output = nn.Linear(n_dims, n_channels)
#         self.scale = n_dims ** -0.5
#         self.n_heads = n_heads
#         self.n_dims = n_dims
...
#         self.lin_cond = nn.Linear(d_cond, n_channels)
#
#         # Condition projection for Key and Value
#         self.cond_proj = nn.Linear(d_cond, n_dims * 2)  # To produce K_cond and V_cond
#
...
#         x = x.view(b, c, h * w).permute(0, 2, 1)  # (B, C, H*W) -> (B, H*W, C)
#
#         # Generate Q, K, V for input features
#         q, k, v = self.proj(x).view(b, h * w, self.n_heads, -1).chunk(3, dim=-1)  # Split into Q, K, V
#
#         # Inject condition embedding into Key and Value
#         cond_embed = self.cond_proj(F.silu(cond))  # (B, n_dims * 2)
#         cond_k, cond_v = cond_embed.view(b, 1, self.n_heads, -1).chunk(2, dim=-1)  # Split into K_cond and V_cond
#
#         # Add condition embeddings to Key and Value
#         k = k + cond_k  # Conditionally augmented Key
#         v = v + cond_v  # Conditionally augmented Value
#
#         # Compute scaled dot-product attention
#         att = torch.einsum('blhd, bmhd -> blmh', q, k) * self.scale  # Attention weights
#         att = torch.softmax(att, dim=-2)  # Normalize along the Key dimension
#         x_ = torch.einsum('blmh, bmhd -> blhd', att, v)  # Attention output
#
#         # Combine heads and project back to original dimensions
#         x_ = self.output(x_.reshape(b, h * w, -1)) + x  # Add residual connection
#         return x_.permute(0, 2, 1).view(b, c, h, w)
...
#===================================================================================================================
class ClassConditionedUnet_S(nn.Module):
...
#===================================================================================================================
class ClassConditionedUnet_B(nn.Module):
    def __init__(self, in_channels:int=1,out_channels:int=1,cond_dim: int = 128, cond_dim2: int=128):
        super().__init__()
        self.encoder = nn.ModuleList([
            SwitchSequential(nn.Conv2d(in_channels, 64, kernel_size=3, padding=1)),
    ...
    def forward(self, x, t, cond,cond2):
        ...
        return x
#===================================================================================================================
class ClassConditionedUnet_L(nn.Module):
    def __init__(self, in_channels:int=1,out_channels:int=1,cond_dim: int = 128, cond_dim2: int=1):
        super().__init__()
        self.encoder = nn.ModuleList([
            SwitchSequential(nn.Conv2d(in_channels, 64, kernel_size=3, padding=1)),
            ...
            SwitchSequential(UNet_ResidualBlock(256, 256),UNet_ResidualBlock(256, 256), UNet_AttentionBlock(256, 8, 120), UNet_AttentionBlock(256, 8, 120)),
            SwitchSequential(nn.Conv2d(256, 256, kernel_size=3, padding=1)),
            SwitchSequential(UNet_ResidualBlock(256, 512),UNet_ResidualBlock(512, 512), UNet_AttentionBlock(512, 8, 120), UNet_AttentionBlock(512, 8, 120)),
            SwitchSequential(UNet_ResidualBlock(512, 512),UNet_ResidualBlock(512, 512), UNet_AttentionBlock(512, 8, 120), UNet_AttentionBlock(512, 8, 120))
        ])
        self.bottleneck = SwitchSequential(
            UNet_ResidualBlock(512, 512),
            UNet_ResidualBlock(512, 512),
            UNet_AttentionBlock(512, 8, 120),
            UNet_AttentionBlock(512, 8, 120),
        )
        self.decoder = nn.ModuleList([
            SwitchSequential(UNet_ResidualBlock(1024, 512),UNet_ResidualBlock(512, 512), UNet_AttentionBlock(512, 8, 120), UNet_AttentionBlock(512, 8, 120)),
            SwitchSequential(UNet_ResidualBlock(1024, 512),UNet_ResidualBlock(512, 512), UNet_AttentionBlock(512, 8, 120), UNet_AttentionBlock(512, 8, 120)),
            SwitchSequential(nn.ConvTranspose2d(768, 512, kernel_size=3, padding=1)),
            SwitchSequential(UNet_ResidualBlock(768, 256),UNet_ResidualBlock(256, 256), UNet_AttentionBlock(256, 8, 120), UNet_AttentionBlock(256, 8, 120)),
            SwitchSequential(UNet_ResidualBlock(512, 256),UNet_ResidualBlock(256, 256), UNet_AttentionBlock(256, 8, 120), UNet_AttentionBlock(256, 8, 120)),
            SwitchSequential(nn.ConvTranspose2d(512, 256, 4, 2, 1)),
            SwitchSequential(UNet_ResidualBlock(384, 128),UNet_ResidualBlock(128, 128), UNet_AttentionBlock(128, 8, 80), UNet_AttentionBlock(128, 8, 80)),
            SwitchSequential(UNet_ResidualBlock(256, 128),UNet_ResidualBlock(128, 128), UNet_AttentionBlock(128, 8, 80), UNet_AttentionBlock(128, 8, 80)),
    ...
    def forward(self, x, t, cond,cond2):
        ...
        return x
#===================================================================================================================
class ClassConditionedUnet_X(nn.Module):
    def __init__(self, in_channels:int=1,out_channels:int=1,cond_dim: int = 128, cond_dim2: int=1):
        super().__init__()
        self.encoder = nn.ModuleList([
            SwitchSequential(nn.Conv2d(in_channels, 64, kernel_size=3, padding=1)),
            ...
            SwitchSequential(UNet_ResidualBlock(256, 256),UNet_ResidualBlock(256, 256), UNet_AttentionBlock(256, 8, 120), UNet_AttentionBlock(256, 8, 120)),
            SwitchSequential(UNet_ResidualBlock(256, 256),UNet_ResidualBlock(256, 256), UNet_AttentionBlock(256, 8, 120), UNet_AttentionBlock(256, 8, 120)),
            SwitchSequential(nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)),
            SwitchSequential(UNet_ResidualBlock(256, 512), UNet_ResidualBlock(512, 512),UNet_AttentionBlock(512, 8, 120), UNet_AttentionBlock(512, 8, 120)),
            SwitchSequential(UNet_ResidualBlock(512, 512), UNet_ResidualBlock(512, 512),UNet_AttentionBlock(512, 8, 120), UNet_AttentionBlock(512, 8, 120))
        ])
        self.bottleneck = SwitchSequential(
