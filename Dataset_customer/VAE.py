...
#===================================================================================================================
class VAE_Encoder_S(nn.Sequential):
    def __init__(self, device,in_channels,z_channels):
        ...
        self.encoder = nn.ModuleList([
            ...
            # output, (B, 8, H/ 4, W/ 4)
            nn.Conv2d(8, z_channels*2, kernel_size= 1, padding= 0)
        ])
...
class VAE_Decoder_S(nn.Sequential):
    def __init__(self,out_channels,z_channels):
        super().__init__()
        self.decoder = nn.ModuleList([
            # (B, 4, H/ 4, W/ 4)-> (B, 128, H/ 4, W/ 4)
            nn.Conv2d(z_channels, 128, kernel_size= 1, padding= 0),
            nn.Conv2d(128, 128, kernel_size= 3, padding= 1),
            VAE_AttentionBlock(128),
            VAE_ResidualBlock(128, 128),
            # (B, 128, H/ 4, W/ 4)-> (B, 128, H/ 2, W/ 2)
            nn.Upsample(scale_factor= 2),
            # (B, 128, H/ 2, W/ 2)-> (B, 64, H/ 2, W/ 2)
            nn.Conv2d(128, 64, kernel_size= 3, padding= 1),
            VAE_ResidualBlock(64, 64),
            # (B, 64, H/ 2, W/ 2)-> (B, 64, H, W)
            nn.Upsample(scale_factor= 2),
...
class VAE_S(nn.Module):
    def __init__(self, device,z_channels):
        super().__init__()
        self.device= device
        self.encoder= VAE_Encoder_S(device=device,in_channels=1,z_channels=z_channels).to(device)
        self.decoder= VAE_Decoder_S(out_channels=1,z_channels=z_channels).to(device)
...
if __name__ == "__main__":
    ...
    # 初始化模型
    model = VAE_M(device=device,z_channels=2)
    # 计算参数量
    total_params = sum(p.numel() for p in model.parameters())
    print(f"总参数量: {total_params}")

    # 输入
    x = torch.randn(8, 1, 128, 128).to(device)  # 输入图像 (B, C, H, W)

    # 前向传播
    mu, sga, decoderz = model(x)

    print(mu.shape, sga.shape, decoderz.shape)  # 输出形状